 ...
 */
function findAllByNamesheetGeneric(props: findAllByNamesheetGeneric): string {
  const { nameSheet, idDatabase } = props;

  const sheet = obtenerSheetGeneric(nameSheet, idDatabase);
  let data: any[];
  if (!sheet)
